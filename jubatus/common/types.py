...
def check_type(value, typ):
    if not isinstance(value, typ):
        raise TypeError('type %s is expected, but %s is given' %
                        (typ, type(value)))
...
class TInt(object):
    ...
    def from_msgpack(self, m):
        ...
        if not (self.min <= m and m <= self.max):
            raise ValueError(
                'int value must be in (%d, %d), but %d is given' % (self.min, self.max, m))
        return m
...
class TTuple(object):
    ...
    def check_tuple(self, m):
        ...
        if len(m) != len(self.types):
            raise TypeError("size of tuple is %d, but %d is expected: %s" % (
                len(m), len(self.types), str(m)))
...
class TUserDef(object):
    ...
    def to_msgpack(self, m):
        if isinstance(m, self.type):
            ...
            raise TypeError(
                'type %s or tuple/list are expected, but %s is given' % (self.type, type(m)))
